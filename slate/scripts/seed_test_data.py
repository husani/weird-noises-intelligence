...
def seed(session, force=False):
    ...
    if force:
        ...
        session.query(SlateShowVersion).delete()
    ...
    session.add_all([
        # Act I
        SlateScene(version_id=vlad_v3.id, act_number=1, scene_number=1, title="Arrival", location="JFK Airport / New York streets", characters_present=["Vlad", "Ensemble"], description="Vlad arrives in America. Culture shock musical number as he navigates customs, taxis, and Times Square.", estimated_minutes=8, sort_order=0),
        SlateScene(version_id=vlad_v3.id, act_number=1, scene_number=2, title="The Diner", location="A diner in Queens", characters_present=["Vlad", "Elena"], description="Vlad and Elena meet. She's running a doomed campaign and he accidentally gives the best stump speech she's ever heard.", estimated_minutes=10, sort_order=1),
        SlateScene(version_id=vlad_v3.id, act_number=1, scene_number=3, title="The Decision", location="Elena's office", characters_present=["Elena", "Vlad"], description="Elena convinces Vlad to run. 'Democracy Is a Bloodsport' begins.", estimated_minutes=6, sort_order=2),
        SlateScene(version_id=vlad_v3.id, act_number=1, scene_number=4, title="The Campaign Trail", location="Various — rallies, diners, TV studios", characters_present=["Vlad", "Elena", "Tucker", "Ensemble"], description="Montage of the campaign. Vlad's radical honesty goes viral.", estimated_minutes=12, sort_order=3),
        SlateScene(version_id=vlad_v3.id, act_number=1, scene_number=5, title="The Debate", location="Debate stage", characters_present=["Vlad", "Senator Hargrove", "Ensemble"], description="Vlad destroys Hargrove in the debate by simply telling the truth. Act I finale — 'Democracy Is a Bloodsport' full company.", estimated_minutes=15, sort_order=4),
        # Act II
        SlateScene(version_id=vlad_v3.id, act_number=2, scene_number=1, title="Victory Night", location="Campaign headquarters", characters_present=["Vlad", "Elena", "Mina", "Tucker", "Ensemble"], description="Vlad wins the primary. Mina confronts him about exposing the family.", estimated_minutes=10, sort_order=5),
        SlateScene(version_id=vlad_v3.id, act_number=2, scene_number=2, title="Father and Daughter", location="Vlad's apartment", characters_present=["Vlad", "Mina"], description="The emotional center. Vlad and Mina's duet about belonging, immortality, and what it means to be seen.", estimated_minutes=8, sort_order=6),
        SlateScene(version_id=vlad_v3.id, act_number=2, scene_number=3, title="The Opposition", location="TV studio / Hargrove's office", characters_present=["Senator Hargrove", "Tucker"], description="Hargrove's team digs into Vlad's past. They discover he's... actually a vampire.", estimated_minutes=7, sort_order=7),
        SlateScene(version_id=vlad_v3.id, act_number=2, scene_number=4, title="Exposure", location="Press conference", characters_present=["Vlad", "Elena", "Ensemble"], description="The truth comes out. Vlad admits everything on live TV. The question becomes: does it matter?", estimated_minutes=10, sort_order=8),
    ...
    # Songs
    session.add_all([
        SlateSong(version_id=vlad_v3.id, title="Fresh Blood", act=1, scene=1, characters=["Vlad", "Ensemble"], song_type="opening", description="Vlad arrives in America. Big ensemble number establishing his wonder and confusion.", sort_order=0),
        SlateSong(version_id=vlad_v3.id, title="The Honest Truth", act=1, scene=2, characters=["Vlad"], song_type="I Want", description="Vlad discovers that honesty is his superpower. His I Want song — he wants to be understood.", sort_order=1),
        SlateSong(version_id=vlad_v3.id, title="Spin Cycle", act=1, scene=4, characters=["Elena", "Ensemble"], song_type="comedy", description="Elena tries to manage Vlad's message. Comedic number about political messaging vs. radical truth.", sort_order=2),
        SlateSong(version_id=vlad_v3.id, title="Democracy Is a Bloodsport", act=1, scene=5, characters=["Vlad", "Ensemble"], song_type="finale", description="Act I finale. Vlad wins the debate. Full company production number.", sort_order=3),
        SlateSong(version_id=vlad_v3.id, title="What They Don't See", act=2, scene=1, characters=["Mina"], song_type="ballad", description="Mina's solo about living in her father's shadow for 500 years.", sort_order=4),
        SlateSong(version_id=vlad_v3.id, title="Old Country", act=2, scene=2, characters=["Vlad", "Mina"], song_type="ballad", description="Father-daughter duet about home, belonging, and the cost of immortality. The emotional core of the show.", sort_order=5),
        SlateSong(version_id=vlad_v3.id, title="Nothing to Hide", act=2, scene=4, characters=["Vlad"], song_type="eleven o'clock", description="Vlad admits everything on live TV. Eleven o'clock number — raw, vulnerable, defiant.", sort_order=6),
        SlateSong(version_id=vlad_v3.id, title="Old Country (Reprise)", act=2, scene=5, characters=["Full Company"], song_type="finale", description="The finale. Callbacks to Act I. What does it mean to choose a country?", sort_order=7),
    ])

    # Emotional arc points
    session.add_all([
        SlateArcPoint(version_id=vlad_v3.id, position=0, intensity=40, label="Arrival — wonder and confusion", tone="playful", sort_order=0),
        SlateArcPoint(version_id=vlad_v3.id, position=8, intensity=55, label="Vlad meets Elena", tone="comedic", sort_order=1),
        SlateArcPoint(version_id=vlad_v3.id, position=15, intensity=45, label="The honest truth — I Want", tone="intimate", sort_order=2),
        SlateArcPoint(version_id=vlad_v3.id, position=25, intensity=60, label="Campaign montage builds", tone="energetic", sort_order=3),
        SlateArcPoint(version_id=vlad_v3.id, position=35, intensity=50, label="Elena's spin cycle", tone="comedic", sort_order=4),
        SlateArcPoint(version_id=vlad_v3.id, position=45, intensity=85, label="The debate — Vlad destroys Hargrove", tone="triumphant", sort_order=5),
        SlateArcPoint(version_id=vlad_v3.id, position=50, intensity=90, label="Act I finale — Democracy Is a Bloodsport", tone="triumphant", sort_order=6),
        SlateArcPoint(version_id=vlad_v3.id, position=55, intensity=65, label="Victory — but Mina confronts Vlad", tone="tense", sort_order=7),
        SlateArcPoint(version_id=vlad_v3.id, position=62, intensity=80, label="Old Country duet — father and daughter", tone="intimate", sort_order=8),
        SlateArcPoint(version_id=vlad_v3.id, position=68, intensity=50, label="Hargrove discovers the truth", tone="ominous", sort_order=9),
        SlateArcPoint(version_id=vlad_v3.id, position=75, intensity=70, label="The exposure — truth comes out", tone="tense", sort_order=10),
        SlateArcPoint(version_id=vlad_v3.id, position=82, intensity=90, label="Nothing to Hide — eleven o'clock", tone="devastating", sort_order=11),
        SlateArcPoint(version_id=vlad_v3.id, position=88, intensity=60, label="Election night uncertainty", tone="tense", sort_order=12),
        SlateArcPoint(version_id=vlad_v3.id, position=95, intensity=85, label="Old Country reprise — finale", tone="triumphant", sort_order=13),
        SlateArcPoint(version_id=vlad_v3.id, position=100, intensity=75, label="Curtain — bittersweet hope", tone="melancholic", sort_order=14),
    ])

    # Runtime estimate
    session.add(SlateRuntimeEstimate(
        version_id=vlad_v3.id, total_minutes=135,
    ...
    session.add(SlateCastRequirements(
        version_id=vlad_v3.id, minimum_cast_size=8, recommended_cast_size=12,
    ...
    session.add(SlateBudgetEstimate(
        version_id=vlad_v3.id, estimated_range="$2.5M-$4M",
    ...
    # Comparables
    session.add_all([
        SlateComparable(version_id=vlad_v3.id, title="Bloody Bloody Andrew Jackson", relationship_type="tonal match", reasoning="Same irreverent approach to politics through genre pastiche. Both use rock-inflected scores to tell the story of an outsider who disrupts the political establishment. Vlad has a warmer emotional core."),
        SlateComparable(version_id=vlad_v3.id, title="The Band's Visit", relationship_type="emotional tone", reasoning="The fish-out-of-water immigrant experience, the quiet moments between the comedy. Vlad's relationship with America has the same tender bewilderment."),
        SlateComparable(version_id=vlad_v3.id, title="What We Do in the Shadows", relationship_type="same audience", reasoning="The vampire-as-immigrant comedy. Vlad's audience already exists — they watch this show. The musical earns its emotional moments in a way the TV show doesn't attempt."),
        SlateComparable(version_id=vlad_v3.id, title="Hamilton", relationship_type="structural parallel", reasoning="Immigrant arrives, enters politics, changes everything. But where Hamilton is earnest, Vlad is satirical. The debate scene functions like the cabinet battles."),
        SlateComparable(version_id=vlad_v3.id, title="The Producers", relationship_type="comedic lineage", reasoning="The tradition of musical comedy that's actually about something. Vlad is funnier than it has any right to be, but the father-daughter relationship gives it genuine stakes."),
    ])

    # Content advisories
    session.add_all([
        SlateContentAdvisory(version_id=vlad_v3.id, category="language", description="Moderate profanity throughout, mostly in political dialogue. Several uses of stronger language in Act II.", severity="moderate"),
        SlateContentAdvisory(version_id=vlad_v3.id, category="violence", description="Stylized vampire references — no graphic violence. Brief stage combat in Act II (the 'exposure' scene). Comedic, not threatening.", severity="mild"),
        SlateContentAdvisory(version_id=vlad_v3.id, category="mature themes", description="Political satire that directly references real political dynamics. Immigration, xenophobia, and media manipulation are central themes.", severity="moderate"),
    ])

    # Logline drafts
    session.add_all([
        SlateLoglineDraft(version_id=vlad_v3.id, text="A 500-year-old vampire emigrates to America, accidentally runs for president, and discovers that radical honesty is the most dangerous weapon in politics.", tone="commercial"),
        SlateLoglineDraft(version_id=vlad_v3.id, text="In a political landscape built on lies, the most honest candidate in history has a secret that should disqualify him from everything — except he's already dead.", tone="literary"),
        SlateLoglineDraft(version_id=vlad_v3.id, text="A father who has lived for five centuries finally finds something worth dying for: the messy, beautiful, impossible promise of democracy.", tone="emotional"),
    ])
