...
import { listVenues, deleteVenue } from '@producers/api'
...
export default function VenuesPage() {
  ...
  function load() {
    ...
    listVenues({ search: debouncedSearch, limit, offset })
      .then(data => { setItems(data.venues || []); setTotal(data.total || 0) })
  ...
  async function handleDelete() {
    ...
    try {
      const result = await deleteVenue(deleteConfirm.id)
      if (result.error) {
  ...
  const COLUMNS = [
    {
      ...
      render: (_, row) => (
        <ActionMenu items={[
          { label: 'Edit', icon: 'M11 1.5l2 2-7.5 7.5H3.5v-2L11 1.5z', onClick: () => navigate(`/producers/venues/${row.id}/edit`) },
          { divider: true },
    ...
    <>
      <div className="page-topbar">
        <div className="page-header">
          ...
          <p className="page-subtitle">{total > 0 ? `${total} venue${total !== 1 ? 's' : ''}` : 'Theatres and performance spaces'}</p>
        </div>
        <div className="page-topbar-actions">
          <Link to="/producers/venues/new" className="btn btn-primary">
      ...
      <div className="filter-row-compact">
        <div className="query-bar search-bar-constrained">
          ...
          <input className="query-input" placeholder="Search venues..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <DataTable
          data={items}
          columns={COLUMNS}
          onRowClick={row => navigate(`/producers/venues/${row.id}`)}
          pagination={{ total, page, limit, onPageChange: setPage, onLimitChange: setLimit }}
          emptyState={
            <EmptyState
              title="No venues found"
              description={search ? 'Try a different search.' : 'Add venues to track theatres and performance spaces.'}
            />
          }
        />
      )}
      ...
      {deleteConfirm && (
        <Modal title="Delete Venue" onClose={() => setDeleteConfirm(null)}
          footer={<>
