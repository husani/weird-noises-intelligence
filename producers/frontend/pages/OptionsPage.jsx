...
import { getAllLookupValues, deleteLookupValue, reorderLookupValues } from '@producers/api'
...
export default function OptionsPage() {
  ...
  async function handleMoveUp(index) {
    ...
    await reorderLookupValues(group.category, group.entity_type, ids)
    load()
  }
  ...
  const COLUMNS = [
    {
      ...
      render: (_, row) => {
        ...
        return (
          <ActionMenu items={[
            { label: 'Edit', icon: 'M11 1.5l2 2-7.5 7.5H3.5v-2L11 1.5z', onClick: () => navigate(`/producers/options/${row.id}/edit`) },
  ...
  const addUrl = `/producers/options/new?category=${encodeURIComponent(group.category)}&entity_type=${encodeURIComponent(group.entity_type)}`
        ...
        <div className="options-workbench-content">
          ...
          {loading ? (
            ...
            <DataTable
              data={filtered}
              columns={COLUMNS}
              onRowClick={row => navigate(`/producers/options/${row.id}/edit`)}
