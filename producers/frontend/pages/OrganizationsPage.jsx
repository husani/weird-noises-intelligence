...
import { listOrganizations, deleteOrganization } from '@producers/api'
...
export default function OrganizationsPage() {
  ...
  function load() {
    ...
    listOrganizations({ search: debouncedSearch, limit, offset })
      .then(d => { setItems(d.organizations || []); setTotal(d.total || 0) })
  ...
  const COLUMNS = [
    {
      ...
      render: (_, row) => (
        <ActionMenu items={[
          { label: 'Edit', icon: 'M11 1.5l2 2-7.5 7.5H3.5v-2L11 1.5z', onClick: () => navigate(`/producers/organizations/${row.id}/edit`) },
          { divider: true },
    ...
    <>
      <div className="page-topbar">
        <div className="page-header">
          ...
          <p className="page-subtitle">{total > 0 ? `${total} organization${total !== 1 ? 's' : ''}` : 'Producing offices, theatres, and companies'}</p>
        </div>
        <div className="page-topbar-actions">
          <Link to="/producers/organizations/new" className="btn btn-primary">
      ...
      <div className="filter-row-compact">
        <div className="query-bar search-bar-constrained">
          ...
          <input className="query-input" placeholder="Search organizations..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>
        ...
        <DataTable
          ...
          emptyState={
            <EmptyState
              ...
              description={search ? 'Try a different search.' : 'Organizations are created when producers are researched, or add them manually.'}
            />
          }
        />
      )}
